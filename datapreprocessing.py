...
import pickle
import os
import re # For cleaning breed/color
...
def parse_age_to_days(age_str):
    ...
    if not num:
        return 0 # Or handle as NaN? Assume 0 if no number found
    num = int(num[0])
    ...
    if 'year' in age_str:
        ...
        return num
    return 0 # Default if unit not recognized
...
def clean_text_feature(text):
    ...
    if pd.isna(text):
        return "unknown"
    text = str(text).lower()
    text = re.sub(r'\s+mix$', '', text) # Remove trailing ' mix'
    text = re.sub(r'\/.*', '', text) # Keep only primary type before '/'
    text = re.sub(r'[^a-z\s]', '', text) # Remove non-alpha chars except space
    text = text.strip()
    if not text:
        return "unknown"
    return text
...
if PROCESS_TRAIN_DATA:
    ...
    # We need the Id col in test df for submission, drop others
    pass # Id handled above
...
# Ensure Outcome Type is not dropped from test data if it exists there by mistake
if not PROCESS_TRAIN_DATA and 'Outcome Type' in cols_to_drop:
    cols_to_drop.remove('Outcome Type')

df = df.drop(columns=[col for col in cols_to_drop if col in df.columns])

# --- Feature Processing ---

# 1. Target Variable (Train only)
if PROCESS_TRAIN_DATA:
    outcome_mapping = {
...
# 4. Categorical Features for Embedding
...
if PROCESS_TRAIN_DATA:
    ...
    for col in cat_embed_cols:
        ...
        # Handle potential NaNs introduced by cleaning before encoding
    ...
    for col in cat_embed_cols:
        ...
        # Handle unseen values in test set -> map to 'unknown' if possible
        df[col + '_encoded'] = df[col].apply(lambda x: le.transform([x])[0] if x in le.classes_ else le.transform(['unknown'])[0])
    df = df.drop(columns=cat_embed_cols)


# 5. Categorical Features for One-Hot Encoding
...
# Example: Fill with median (or mean) - fit scaler *after* this
...
if PROCESS_TRAIN_DATA:
    imputation_values = df[numerical_cols].median() # Use median for robustness to outliers
...
# --- Final Check & Save ---
# Align columns between train/test (important after OHE)
if PROCESS_TRAIN_DATA:
